    ...
    <section class="max-w-7xl mx-auto px-4 py-10">
        @if(isset($banners) && $banners->count())
            <div x-data="{ i: 0, t: null, total: {{ $banners->count() }} }"
                x-init="t = setInterval(() => { i = (i + 1) % total }, 5000)" class="relative overflow-hidden rounded-2xl">
                @foreach ($banners as $k => $b)
                    <a href="{{ $b->url ?? route('shop.sale') }}" class="block" x-show="i === {{ $k }}" x-transition.opacity
                        x-cloak>
